...
podman rmi nc_web nc_app nc_openssl
...
envsubst <web/nginx.in.conf >web/nginx.conf \
  '${SERVER_EXTERNAL_IP} ${SERVER_FQ_DOMAINS} ${SERVER_HTTP_PORT} ${SERVER_HTTPS_PORT} ${DNS_RESOLVER_IP} ${NC_APP_NAME} ${WP_APP_NAME}'
...
podman pod rm ${CONTAINER_FORCE} nc || true
